...
for id, url in tqdm(
    sql_queries.article_records_not_in_table(columns=["link_article"])
):
    ...
    if soup:
        ...
        try:
            headline_text = (
                headline_html.find("h1").text.encode("latin").decode()
            )
        except AttributeError:
            print(
                "Headline has not been identified from the implemented rules."
            )
            print(headline_html)
            continue
        ...
        try:
            teaser_text = (
                teaser_html.find_all("div")[-1].text.encode("latin").decode()
            )
        except AttributeError:
        ...
        content_text = html_to_text.cut_text_at_phrase(
            text=html_to_text.html_to_text(content_html),
            phrase="Weitere News zum Thema",
        )
